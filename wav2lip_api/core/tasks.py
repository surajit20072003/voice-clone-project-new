...
# --- Models ko global scope mein load karein (Optimization) ---
# Models ko sirf ek baar load kiya jayega jab worker shuru hoga
try:
    from TTS.api import TTS
    TTS_MODEL = TTS(model_name="tts_models/multilingual/multi-dataset/your_tts", progress_bar=False, gpu=True)
    logger.info("TTS Model loaded successfully at startup.")
except Exception as e:
    TTS_MODEL = None
    logger.error(f"Failed to load TTS model at startup: {e}", exc_info=True)

try:
    import ollama as ol
    OLLAMA_CLIENT = ol
    OLLAMA_CLIENT.chat(model='llama3', messages=[{'role': 'user', 'content': 'Test LLaMA3'}])
    logger.info("Ollama Client connected successfully at startup.")
except Exception as e:
    OLLAMA_CLIENT = None
    logger.error(f"Failed to connect to Ollama at startup: {e}", exc_info=True)
